...
- Summary line: exactly one sentence, at the **same font size as the body text**. A larger summary font was tried once and reverted — keep it at body size unless explicitly asked to change it again.
- The one-sentence summary must stay on a single visual line, not wrap. If a wording change (or any font-size experiment) pushes it to 2 lines, first try trimming connective words to pull it back to one line — don't cut substantive content, and don't silently leave it wrapped.
